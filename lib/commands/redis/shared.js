...
function make_addons_filter(filter) {
  ...
  function on_response(addons) {
    ...
    for (var i=0; i < addons.length; i++) {
      ...
      if (service.indexOf(addon_name()) === 0 && (!filter || matches(addon))) {
...
function addon_name() {
  if (process.env.HEROKU_REDIS_ADDON_NAME) {
    return process.env.HEROKU_REDIS_ADDON_NAME;
  }
  return ADDON;
}

function api_host_name() {
  if (process.env.HEROKU_REDIS_HOST) {
    return process.env.HEROKU_REDIS_HOST + '.herokuapp.com';
  }
  return HOST;
}
