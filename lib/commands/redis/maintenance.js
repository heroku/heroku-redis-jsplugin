...
module.exports = {
  ...
  run: cli.command(function* (context, heroku) {
    ...
    let addon = addons[0];
    let redis_api_path = `${api.CLIENT_PATH_BASE}/${addon.name}`;
    ...
    if (context.flags.window) {
      ...
      let maintenance = yield api.request(context, `${redis_api_path}/maintenance_window`, 'PUT', { description: context.flags.window });
    ...
    if (context.flags.run) {
      ...
      let maintenance = yield api.request(context, `${redis_api_path}/maintenance`, 'POST');
      console.log(maintenance.message);
      process.exit(0);
    }

    let maintenance = yield api.request(context, `${redis_api_path}/maintenance`, 'GET', null);
    console.log(maintenance.message);
  })
};
