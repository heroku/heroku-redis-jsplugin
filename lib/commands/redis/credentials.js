...
module.exports = {
  ...
  run: cli.command(function* (context, heroku) {
    ...
    let addon = addons[0];
    let redis_api_path = `${api.REDIS_PATH_BASE}/${addon.name}`;
    if (context.flags.reset) {
      console.log(`Resetting credentials for ${addon.name}`);
      yield api.request(context, `${redis_api_path}/credentials_rotation`, 'POST');
    } else {
      let redis = yield api.request(context, redis_api_path);
      if (addons.length === 0) {
        cli.error('No Redis instances found.');
        process.exit(1);
      } else {
